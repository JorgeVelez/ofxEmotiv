...
SocketServer::SocketServer(int port, int connections, SocketStream stream, TypeSocket type) {
...
SocketClient::SocketClient(const string& host, int port, SocketStream stream) : Socket(stream) {
	...
	if (::connect(s_, (sockaddr *) &addr, sizeof(sockaddr))) {
		error << "cannot connection to host [" << host << "] on port " << port << ".";
		throw SocketException(error.str());
	}
}
